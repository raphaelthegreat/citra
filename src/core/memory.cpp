...
public:
    ...
    std::vector<Common::PageTable*> page_table_list;
    ...
    void ReadBlockImpl(Common::PageTable& page_table, const VAddr src_addr, void* dest_buffer,
    ...
    template <bool UNSAFE>
    void WriteBlockImpl(Common::PageTable& page_table, const VAddr dest_addr,
    ...
    u8* GetPointerForRasterizerCache(VAddr addr) {
        ...
        if (addr >= PLUGIN_3GX_FB_VADDR && addr < PLUGIN_3GX_FB_VADDR_END) {
            ...
            if (plg_ldr) {
                return fcram.get() + addr - PLUGIN_3GX_FB_VADDR + plg_ldr->GetPluginFBAddr() - FCRAM_PADDR;
            }
        }
...
void MemorySystem::SetCurrentPageTable(Common::PageTable* page_table) {
    impl->current_page_table = page_table;
}

Common::PageTable* MemorySystem::GetCurrentPageTable() const {
...
void MemorySystem::MapPages(Common::PageTable& page_table, VAddr base, u32 size, MemoryRef memory,
    ...
    while (base != end) {
        ...
        if (type == Common::PageType::Memory && impl->cache_marker.IsCached(base * CITRA_PAGE_SIZE)) {
            page_table.pointers[base] = nullptr;
            page_table.attributes[base] = Common::PageType::RasterizerCachedMemory;
        }
...
void MemorySystem::MapMemoryRegion(Common::PageTable& page_table, VAddr base, u32 size, MemoryRef target) {
    ...
    MapPages(page_table, base / CITRA_PAGE_SIZE, size / CITRA_PAGE_SIZE, target, Common::PageType::Memory);
}

void MemorySystem::UnmapRegion(Common::PageTable& page_table, VAddr base, u32 size) {
    ASSERT_MSG((size & CITRA_PAGE_MASK) == 0, "non-page aligned size: {:08X}", size);
    ASSERT_MSG((base & CITRA_PAGE_MASK) == 0, "non-page aligned base: {:08X}", base);
...
void MemorySystem::RegisterPageTable(Common::PageTable* page_table) {
    impl->page_table_list.push_back(page_table);
}

void MemorySystem::UnregisterPageTable(Common::PageTable* page_table) {
    auto it = std::ranges::find(impl->page_table_list, page_table);
...
T MemorySystem::Read(const VAddr vaddr) {
    ...
    auto page_type = impl->current_page_table->attributes[vaddr >> CITRA_PAGE_BITS];
    switch (page_type) {
    case Common::PageType::Unmapped:
...
void MemorySystem::Write(const VAddr vaddr, const T data) {
    ...
    const auto page_type = impl->current_page_table->attributes[vaddr >> CITRA_PAGE_BITS];
    switch (page_type) {
    case Common::PageType::Unmapped:
        ...
        return;
    case Common::PageType::Memory:
        UNREACHABLE_MSG("Mapped memory page without a pointer @ {:08X}", vaddr);
        break;
    case Common::PageType::RasterizerCachedMemory: {
...
bool MemorySystem::IsValidVirtualAddress(const Kernel::Process& process, const VAddr vaddr) {
    ...
    if (page_table.attributes[vaddr >> CITRA_PAGE_BITS] == Common::PageType::RasterizerCachedMemory) {
        return true;
    }
...
std::span<u8> MemorySystem::GetPhysicalSpan(PAddr address) const {
    ...
    return std::span{target_mem, offset_into_region};
}
...
void MemorySystem::RasterizerMarkRegionCached(PAddr start, u32 size, bool cached) {
    ...
    for (u32 i = 0; i < num_pages; ++i, paddr += CITRA_PAGE_SIZE) {
        for (VAddr vaddr : PhysicalToVirtualAddressForRasterizer(paddr)) {
            impl->cache_marker.Mark(vaddr, cached);
            for (auto& page_table : impl->page_table_list) {
                auto& page_type = page_table->attributes[vaddr >> CITRA_PAGE_BITS];
                if (cached) {
...
void MemorySystem::ReadBlock(Common::PageTable& page_table, const VAddr src_addr,
                             void* dest_buffer, const std::size_t size) {
    return impl->ReadBlockImpl<false>(page_table, src_addr, dest_buffer, size);
}

void MemorySystem::ReadBlock(VAddr src_addr, void* dest_buffer, std::size_t size) {
    return impl->ReadBlockImpl<false>(*impl->current_page_table, src_addr, dest_buffer, size);
}
...
void MemorySystem::WriteBlock(Common::PageTable& page_table, const VAddr dest_addr,
                              const void* src_buffer, const std::size_t size) {
    return impl->WriteBlockImpl<false>(page_table, dest_addr, src_buffer, size);
}

void MemorySystem::WriteBlock(const VAddr dest_addr, const void* src_buffer,
                              const std::size_t size) {
    return impl->WriteBlockImpl<false>(*impl->current_page_table, dest_addr, src_buffer, size);
}

void MemorySystem::ZeroBlock(Common::PageTable& page_table, const VAddr dest_addr,
                             const std::size_t size) {
    std::size_t remaining_size = size;
    std::size_t page_index = dest_addr >> CITRA_PAGE_BITS;
    std::size_t page_offset = dest_addr & CITRA_PAGE_MASK;
    ...
    while (remaining_size > 0) {
        ...
        case Common::PageType::Memory: {
            u8* pointer = page_table.pointers[page_index] + page_offset;
            std::memset(pointer, 0, copy_amount);
            break;
        }
        case Common::PageType::RasterizerCachedMemory: {
            RasterizerFlushVirtualRegion(current_vaddr, static_cast<u32>(copy_amount),
...
void MemorySystem::CopyBlock(Common::PageTable& page_table, VAddr dest_addr, VAddr src_addr,
                             const std::size_t size) {
    CopyBlock(page_table, page_table, dest_addr, src_addr, size);
}

void MemorySystem::CopyBlock(Common::PageTable& dest_page_table,
                             Common::PageTable& src_page_table, VAddr dest_addr, VAddr src_addr,
                             std::size_t size) {
    std::size_t remaining_size = size;
    std::size_t page_index = src_addr >> CITRA_PAGE_BITS;
    std::size_t page_offset = src_addr & CITRA_PAGE_MASK;
    ...
    while (remaining_size > 0) {
        ...
        switch (src_page_table.attributes[page_index]) {
        case Common::PageType::Unmapped: {
            LOG_ERROR(HW_Memory,
                      "unmapped CopyBlock @ 0x{:08X} (start address = 0x{:08X}, size = {}) at PC "
                      "0x{:08X}",
                      current_vaddr, src_addr, size, impl->GetPC());
            ZeroBlock(dest_page_table, dest_addr, copy_amount);
            break;
        }
        case Common::PageType::Memory: {
            const u8* src_ptr = src_page_table.pointers[page_index] + page_offset;
            WriteBlock(dest_page_table, dest_addr, src_ptr, copy_amount);
            break;
        }
        case Common::PageType::RasterizerCachedMemory: {
            RasterizerFlushVirtualRegion(current_vaddr, static_cast<u32>(copy_amount),
                                         FlushMode::Flush);
            WriteBlock(dest_page_table, dest_addr, GetPointerForRasterizerCache(current_vaddr),
                       copy_amount);
            break;
        }
        default:
